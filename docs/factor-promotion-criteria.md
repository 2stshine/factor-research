...
`fr-3.7.0`은 다음 기준을 결과를 보기 전에 고정한다.
...
| 봉인 OOS | 미노출 역사 구간의 정확한 36 signal개월에서 유효한 투자 가능 Rank IC 36개, 평균 `>= 0.05`, 자동 확인 대상 전체 `BY q <= 0.10` | Gold 효과선·일회성 confirmation |
...
월별 IC 표준편차가 `0.07`이면 36개월 평균의 단순 IID 표준오차는 약 `0.0117`이다. 관측 평균
`0.05`의 단측 90% 하한은 약 `0.035`이므로 단순한 0과의 구별을 넘어 비교적 일관된 효과를
요구한다. 그러나 실제 IC는 시간에 따라 변하고 월별 자기상관도 있을 수 있으므로 단순 IID
신뢰구간을 판정에 직접 사용하지 않고 HAC와 자동 확인 대상 전체의 BY를 적용한다.

봉인 OOS는 정확히 36 signal개월이고 유효 IC도 정확히 36개여야 한다. 시작·종료월은 campaign
생성 시 최신 완료 수익률월에서 역산해 고정하며 늦게 공개해도 뒤의 월을 덧붙이지 않는다.
...
7. **진짜 OOS**: 미노출 역사 구간에서 자동 기준을 통과한 전 후보를 정확한 36개월 동안 한 번만 확인하고, 유효 IC 36개·평균 `>= 0.05`와 OOS BY를 함께 요구한다.
8. **구현 동등성**: 전 후보의 query-only Gold SQL을 연구 정의와 hash로 묶고 discovery 구간의 key·raw value·rank parity를 통과한다.
9. **신규성·운용성**: Gold와 거의 같은 신호가 아니며, 통과 후 비용·용량·회전율을 따로 검토한다.
