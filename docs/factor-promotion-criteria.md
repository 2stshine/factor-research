...
`fr-3.5.0`은 다음 기준을 결과를 보기 전에 고정한다.
...
| 시장구분·유동성·비의도 규모 노출 제거 후 투자 가능 Rank IC | `>= 0.01` | soft robustness; size category는 규모 노출 보존, HAC p는 진단 |
| Discovery 다중검정 | campaign의 모든 등록 후보에 `BY q <= 0.10` | 선택 편향 hard gate |
| 봉인 OOS | 고정 60 signal개월에서 유효한 투자 가능 Rank IC 60개, 평균 `>= 0.05`, survivor 전체 `BY q <= 0.10` | Gold 효과선·일회성 confirmation |
| 기존 Gold와 신호 상관 | 월별 절대 Spearman 상관의 중앙값 `<= 0.80` | 중복 hard gate |

수익률, net IR, 회전율과 거래비용은 Gold에 **예측 신호를 등록하는 기준이 아니라** 운용 가능성
진단이다. Discovery `0.03`이나 OOS `0.05` 하나만 넘는다고 합격하지 않으며, 데이터
...
## Discovery 0.03과 Gold OOS 0.05를 선택한 이유
...
상관 방식을 Pearson으로 해석하고 이변량 정규·Gaussian copula를 가정하면
`Spearman = 6/pi * asin(Pearson/2)`다. 이 조건부 환산에서 Pearson `0.05`는 Spearman
`0.0478`, Pearson `0.10`은 Spearman `0.0955`다. Rank 상관이라는 이유만으로 good 기준을
`0.02`나 `0.03`까지 낮출 근거는 없다. 운영상 반올림한 `0.05`를 봉인 OOS Gold 효과선으로
사용하고, `0.10` 이상은 자동 신뢰하지 않고 누수·PIT·표본 오류를 먼저 재점검한다.

다만 현재 레이블은 잔차수익률이 아니라 총수익률 순위이고, T3 중립화도 신호 순위를
시장구분·유동성·비의도 규모 노출에 대해 잔차화할 뿐 미래수익률 자체를 위험모형으로
잔차화하지 않는다. 그러므로 `0.05`는 **MSCI에서 영감을 받은 보수적 정책값**이지
MSCI-equivalent 통계량이라는 주장은 하지 않는다.

### Discovery는 0.03에서 후보를 보존한다

Discovery `0.03`은 Gold 충분조건이나 최소 실질효과가 아니라, 진짜 IC가 `0.05`인 후보를 개발
표본의 추정오차만으로 너무 일찍 버리지 않기 위한 후보선이다. 월별 IC 표준편차 `0.07`, 60개월
IID 근사에서 평균의 표준오차는 약 `0.0090`이고, 진짜 평균이 `0.05`일 때 관측 평균이 `0.03`
미만일 확률은 약 1.3%다. 실제 판정에는 자기상관을 반영한 HAC와 campaign BY를 적용하므로 이
계산은 설계 sanity check일 뿐 보장된 검정력이 아니다.

| 월평균 Rank IC | 이 레포의 해석 |
|---:|---|
| `< 0.01` | 효과가 매우 작음 |
| `0.01 ~ 0.03` | 약한 신호; discovery 후보선 미달 |
| `0.03 ~ 0.05` | discovery 후보 구간; Gold OOS 효과선 미달 |
| `0.05 ~ 0.10` | Gold 효과선 통과 가능 구간; 나머지 gate 확인 필수 |
...
### OOS 60개월은 효과크기와 시간 안정성을 함께 확인한다

월별 IC 표준편차가 `0.07`이면 60개월 평균의 단순 IID 표준오차는 약 `0.0090`이다. 관측 평균
`0.05`의 단측 90% 하한은 약 `0.038`이므로 단순한 0과의 구별을 넘어 비교적 일관된 효과를
요구한다. 그러나 실제 IC는 시간에 따라 변하고 월별 자기상관도 있을 수 있으므로 단순 IID
신뢰구간을 판정에 직접 사용하지 않고 HAC와 survivor BY를 적용한다.

봉인 OOS는 60 signal개월로 고정하고 유효 IC 60개를 요구하며 시작·종료월을 campaign 생성 시 고정한다. 늦게
공개하더라도 뒤의 월을 추가해 판정을 바꾸지 않으며, 마지막 수익률 월이 닫혔음을 다음 달
관측으로 확인한 뒤 공개한다. 그 확정 월은 OOS 경계에서 사라진 종목의 45일 비활성 여부를
판정하는 데만 사용하고, IC 수익률 행에는 넣지 않는다. 60개월은 이 단순 검출 가능성 계산에
따른 **기간 하한**이지, 실제 복합 승격 규칙의 검정력이 특정 값이라는 뜻이 아니다. 실제 규칙은 관측
평균 `>= 0.05`와 survivor BY까지 함께 요구하므로 검정력은 family 크기·자기상관에 따라
달라진다. 고정 60 signal개월 중 유효 IC가 하나라도 빠져 60개가 안 되면 실패한다. 레거시 후보
분포를 보고 기간을 다시 줄이지 않는다.
...
한 family로 묶어 freeze 시점에 한 번만** 계산한다. 그래야 뒤에 후보가 추가됐다는 이유로 과거
판정이 조용히 바뀌지 않는다. 다만 같은 개발 데이터를 본 성찰로 다음 epoch을 만드는 과정은
적응적 연구이므로 discovery q값만을 순수한 확인 검정으로 과장하지 않는다. confirmation은
survivor와 definition hash를 먼저 동결하고 새 OOS를 한 번만 공개한다.
...
5. **공통노출 이후 잔존성**: 시장구분·유동성과 비의도 규모 노출 제거 후에도 신호가 남는다.
   size category에서는 가설 자체인 규모 노출을 보존한다.
6. **선택 편향 통제**: 성공·실패를 포함한 campaign 전체 후보의 HAC p값을 BY 보정한다.
7. **진짜 OOS**: survivor를 먼저 동결하고 고정 구간의 유효 IC 60개와 평균 `>= 0.05`를 확인한 뒤 한 번만 공개하며 OOS도 함께 BY 보정한다.
8. **신규성·구현성**: Gold와 거의 같은 신호가 아니며, 통과 후 비용·용량·회전율을 따로 검토한다.
