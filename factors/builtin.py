...
2026-07-31 첫 실행 결과(로컬 bronze 2015-01-02~2026-07-07, 투자가능 유니버스):
...
NEEDS = ("total_equity", "total_assets", "total_liabilities",
         "net_income_ttm", "revenue_ttm", "operating_income_ttm", "net_income")


def _add(**kw):
    REGISTRY.add(Factor(needs=NEEDS, **kw))
...
_add(
    ...
    compute=lambda d: (d.groupby("Code")["adj_close"].shift(1)
                       / d.groupby("Code")["adj_close"].shift(12) - 1),
)
_add(
    ...
    compute=lambda d: d["adj_close"] / d.groupby("Code")["adj_close"].shift(1) - 1,
)
...
_add(
    ...
    compute=lambda d: (
        d.groupby("Code")["net_income"].diff(4)
        / d.groupby("Code")["net_income"].diff(4)
           .groupby(d["Code"]).transform(lambda s: s.rolling(8, min_periods=4).std())
           .replace(0, np.nan)
    ),
)
