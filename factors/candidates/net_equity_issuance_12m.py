...
def compute(frame):
    ...
    total_return_index = ordered["return_close"].where(ordered["return_close"] > 0)
    market_cap_growth = market_cap / market_cap.groupby(asset).shift(LOOKBACK_MONTHS)
    total_return_growth = total_return_index / total_return_index.groupby(asset).shift(LOOKBACK_MONTHS)
    issuance = market_cap_growth / total_return_growth - 1
...
RESEARCH_SPEC = {
    "thesis": (
        "Silver PIT에서 12개월 시가총액 성장률을 같은 기간 총주주수익 성장률로 나눈 순주식 발행 "
    ...
    "data_notes": (
        "Silver PIT market_cap과 total_return_close에 매핑된 return_close를 사용한다. 12개월 이력이 "
