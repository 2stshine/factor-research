...
def compute(frame):
    ...
    ratio = ordered['revenue_ttm'] / (ordered["market_cap"] + ordered["total_liabilities"]).where((ordered["market_cap"] + ordered["total_liabilities"]) > 0)
...
FACTOR = Factor(
    name='enterprise_sales_yield_change_12m',
    family='enterprise_sales_yield_change_12m',
    category='value',
    exploration_domain='value',
    hypothesis='revenue_ttm 대비 시장가치의 12개월 개선이 큰 기업은 펀더멘털 대비 가격이 덜 반영되어 이후 상대수익이 높다.',
    predicted_sign=1,
...
RESEARCH_SPEC = {
    "thesis": 'revenue_ttm 대비 시장가치의 12개월 개선이 큰 기업은 펀더멘털 대비 가격이 덜 반영되어 이후 상대수익이 높다.',
