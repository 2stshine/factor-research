...
def compute(frame):
    ...
    ratio = ordered['net_income_ttm'] / ordered["market_cap"].where(ordered["market_cap"] > 0)
...
FACTOR = Factor(
    name='earnings_yield_change_12m',
    family='earnings_yield_change_12m',
    category='value',
    exploration_domain='value',
    hypothesis='net_income_ttm 대비 시장가치의 12개월 개선이 큰 기업은 펀더멘털 대비 가격이 덜 반영되어 이후 상대수익이 높다.',
    predicted_sign=1,
    params={'lookback_months': LOOKBACK_MONTHS, 'numerator': 'net_income_ttm', 'denominator': 'market_cap'},
    rebalance_months=3,
