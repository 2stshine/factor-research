...
def compute(frame):
    ...
    ratio = ordered['total_equity'] / ordered["market_cap"].where(ordered["market_cap"] > 0)
...
FACTOR = Factor(
    name='book_to_market_change_6m',
    family='book_to_market_change_6m',
    category='value',
    exploration_domain='value',
    hypothesis='total_equity 대비 시장가치의 6개월 개선이 큰 기업은 펀더멘털 대비 가격이 덜 반영되어 이후 상대수익이 높다.',
    predicted_sign=1,
    params={'lookback_months': LOOKBACK_MONTHS, 'numerator': 'total_equity', 'denominator': 'market_cap'},
    rebalance_months=3,
    needs=('total_equity',),
    compute=compute,
)
...
RESEARCH_SPEC = {
    "thesis": 'total_equity 대비 시장가치의 6개월 개선이 큰 기업은 펀더멘털 대비 가격이 덜 반영되어 이후 상대수익이 높다.',
