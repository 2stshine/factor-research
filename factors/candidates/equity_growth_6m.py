...
FACTOR = Factor(
    name="equity_growth_6m",
    family="equity_growth_6m",
    category="other",
    exploration_domain="financing_issuance",
    hypothesis=(
        "최근 6개월 book_equity 확대가 큰 기업은 외부자금 수요나 "
        "고평가 활용 가능성이 높아 이후 상대수익이 낮다."
    ),
    predicted_sign=-1,
...
RESEARCH_SPEC = {
    ...
    "mechanism": (
        "발행·부채조달·자본금 변화 중 하나를 PIT 시점에서 분리하여 "
        "경영자의 자금조달 결정을 측정한다."
    ),
    "falsification": (
    ...
    "data_notes": (
        "정확한 달력 시차와 양의 분모만 사용하며 기업행사 후행 라벨은 사용하지 않는다."
    ),
}
