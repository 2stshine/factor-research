...
def compute(frame):
    ...
    ratio = ordered['net_income_ttm'] / ordered['revenue_ttm'].where(ordered['revenue_ttm'] != 0)
...
FACTOR = Factor(
    name='net_margin_change_6m',
    family='net_margin_change_6m',
    category='earnings',
    exploration_domain='profitability_quality',
    hypothesis='최근 6개월 net_income_ttm/revenue_ttm 개선이 큰 기업은 이익의 질과 지속성이 높아 이후 상대수익이 높다.',
    predicted_sign=1,
    params={'lookback_months': LOOKBACK_MONTHS, 'numerator': 'net_income_ttm', 'denominator': 'revenue_ttm'},
    rebalance_months=3,
...
RESEARCH_SPEC = {
    "thesis": '최근 6개월 net_income_ttm/revenue_ttm 개선이 큰 기업은 이익의 질과 지속성이 높아 이후 상대수익이 높다.',
