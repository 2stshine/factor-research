...
def compute(frame):
    ...
    ratio = ordered['net_income_ttm'] / (ordered["market_cap"] + ordered["total_liabilities"]).where((ordered["market_cap"] + ordered["total_liabilities"]) > 0)
...
FACTOR = Factor(
    name='enterprise_earnings_yield_change_12m',
    family='enterprise_earnings_yield_change_12m',
    category='value',
    exploration_domain='value',
    hypothesis='net_income_ttm 대비 시장가치의 12개월 개선이 큰 기업은 펀더멘털 대비 가격이 덜 반영되어 이후 상대수익이 높다.',
    predicted_sign=1,
    params={'lookback_months': LOOKBACK_MONTHS, 'numerator': 'net_income_ttm', 'denominator': 'enterprise_value'},
    rebalance_months=3,
...
RESEARCH_SPEC = {
    "thesis": 'net_income_ttm 대비 시장가치의 12개월 개선이 큰 기업은 펀더멘털 대비 가격이 덜 반영되어 이후 상대수익이 높다.',
