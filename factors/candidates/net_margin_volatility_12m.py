...
def compute(frame):
    ...
    source = ordered['net_income_ttm'] / ordered["revenue_ttm"].where(ordered["revenue_ttm"] != 0)
...
FACTOR = Factor(
    name='net_margin_volatility_12m',
    family='net_margin_volatility_12m',
    category='earnings',
    exploration_domain='profitability_quality',
    hypothesis='net_margin_volatility 신호가 낮은 기업은 보고이익의 지속성과 현금전환이 높아 이후 상대수익이 높다.',
    predicted_sign=-1,
    params={'window_months': WINDOW_MONTHS, 'min_observations': MIN_OBSERVATIONS, 'quality_measure': 'net_margin_volatility'},
    rebalance_months=3,
...
RESEARCH_SPEC = {
    "thesis": 'net_margin_volatility 신호가 낮은 기업은 보고이익의 지속성과 현금전환이 높아 이후 상대수익이 높다.',
