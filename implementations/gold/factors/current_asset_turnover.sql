-- current_asset_turnover Gold implementation.
-- value = 신호일에 알려진 최근 4개 독립 분기 매출 합 / 최신 양의 유동자산
...
WITH price_history AS (
    ...
    JOIN public.fundamental f
     ...
     AND f.metric IN ('revenue', 'current_assets')
     AND f.source = 'DART'
    ...
    FROM selected
    WHERE metric = 'current_assets'
), latest_liabilities AS (
    SELECT asset_id, as_of_date, value AS current_assets
    ...
    SELECT
        ...
        operating_ttm.revenue_ttm
            / latest_liabilities.current_assets AS value,
    ...
    WHERE latest_liabilities.current_assets > 0
), ranked AS (
    SELECT
