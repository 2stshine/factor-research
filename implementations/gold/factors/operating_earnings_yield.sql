-- operating_earnings_yield Gold implementation.
-- value = 신호일에 알려진 최근 4개 독립 분기 영업이익 합 / 동월 양의 시가총액
...
WITH price_history AS (
    ...
    SELECT
        ...
        operating_ttm.operating_income_ttm / universe.market_cap AS value,
        operating_ttm.signal_month
    FROM operating_ttm
    JOIN universe
      ON universe.asset_id = operating_ttm.asset_id
     AND universe.as_of_date = operating_ttm.as_of_date
    WHERE universe.market_cap > 0
), ranked AS (
    SELECT
