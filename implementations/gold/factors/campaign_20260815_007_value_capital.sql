-- Query-only shared implementation for campaign-20260815-007 qualifiers.
-- The DART PIT event stream and monthly price universe are materialized once.
WITH query_bounds AS (
    ...
    JOIN public.dq_run q
     ...
     AND q.status = 'CERTIFIED'
    WHERE f.metric IN ('total_equity', 'capital_stock', 'total_assets')
      AND f.source = 'DART'
    ...
    SELECT
        ...
        s.signal_month,
        s.total_equity,
        s.total_equity / m.market_cap AS book_to_market,
        s.capital_stock,
        s.total_assets,
        lag(s.total_equity / m.market_cap, 12) OVER (asset_months)
            AS prior_book_to_market,
        lag(s.signal_month, 12) OVER (asset_months) AS prior_signal_month
    FROM latest_stocks s
    JOIN factor_monthly m
      ON m.asset_id = s.asset_id
     AND m.trade_date = s.as_of_date
    WHERE m.market_cap > 0
    WINDOW asset_months AS (
        PARTITION BY s.asset_id ORDER BY s.signal_month
    )
), raw_values AS (
    SELECT
        'book_to_market_change_12m'::text AS factor,
        values.asset_id,
        values.as_of_date,
        values.signal_month,
        values.book_to_market - values.prior_book_to_market AS value,
    ...
    WHERE values.signal_month BETWEEN bounds.output_start AND bounds.output_end
      AND values.total_equity IS NOT NULL
      AND values.prior_book_to_market IS NOT NULL
      AND values.signal_month = values.prior_signal_month + INTERVAL '12 months'
    UNION ALL
    SELECT
        ...
        values.as_of_date,
        values.signal_month,
        values.capital_stock / values.total_assets,
        -1
    FROM monthly_values values
    JOIN output_universe u
      ON u.asset_id = values.asset_id
    ...
    CROSS JOIN query_bounds bounds
    WHERE values.signal_month BETWEEN bounds.output_start AND bounds.output_end
      AND values.capital_stock IS NOT NULL
      AND values.total_assets > 0
), ranked AS (
    SELECT
        factor,
        asset_id,
