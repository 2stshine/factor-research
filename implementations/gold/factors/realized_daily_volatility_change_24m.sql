-- Query-only implementation for realized_daily_volatility_change_24m.
WITH query_bounds AS (
    SELECT
        (%(start_month)s::date - INTERVAL '37 months')::date AS history_start,
    ...
    SELECT
        monthly.*,
        lag(daily_volatility_252d, 24) OVER (asset_months)
            AS prior_daily_volatility,
        lag(signal_month, 24) OVER (asset_months) AS prior_signal_month
    FROM monthly
    ...
    FROM universe
    WHERE daily_volatility_252d IS NOT NULL
      AND prior_daily_volatility > 0
      AND signal_month = prior_signal_month + INTERVAL '24 months'
), ranked AS (
    SELECT
        asset_id,
        as_of_date,
