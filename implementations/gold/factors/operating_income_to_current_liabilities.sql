-- operating_income_to_current_liabilities Gold implementation.
-- value = 신호일에 알려진 최근 4개 독립 분기 영업이익 합 / 최신 양의 유동부채
...
WITH price_history AS (
    ...
    JOIN public.fundamental f
     ...
     AND f.metric IN ('operating_income', 'current_liabilities')
     AND f.source = 'DART'
    ...
    FROM selected
    WHERE metric = 'current_liabilities'
), latest_liabilities AS (
    SELECT asset_id, as_of_date, value AS current_liabilities
    ...
    SELECT
        ...
        operating_ttm.operating_income_ttm
            / latest_liabilities.current_liabilities AS value,
    ...
    WHERE latest_liabilities.current_liabilities > 0
), ranked AS (
    SELECT
