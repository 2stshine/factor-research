-- Query-only legal-capital growth implementation for campaign-20260815-002.
-- value = exact 12-month growth in the latest DART PIT capital_stock value.
WITH query_bounds AS (
    ...
    SELECT
        ...
        as_of_date,
        signal_month,
        value AS capital_stock
    FROM stock_candidates
    WHERE metric_rank = 1
), lagged AS (
    SELECT
        latest_stocks.*,
        lag(capital_stock, 12) OVER (
            PARTITION BY asset_id ORDER BY signal_month
        ) AS prior_capital_stock,
    ...
    WHERE lagged.signal_month BETWEEN bounds.output_start AND bounds.output_end
      ...
      AND lagged.signal_month = lagged.prior_signal_month + INTERVAL '12 months'
), ranked AS (
    SELECT
        asset_id,
        as_of_date,
