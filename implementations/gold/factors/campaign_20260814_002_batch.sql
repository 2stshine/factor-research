-- Query-only accounting implementation shared by four campaign-20260814-002
-- qualifiers.  The expensive DART PIT event stream is materialized once for
-- the complete discovery interval instead of once per result chunk.
WITH query_bounds AS (
    ...
    JOIN public.dq_run q
     ...
     AND q.status = 'CERTIFIED'
    WHERE f.metric IN (
        'revenue', 'operating_income', 'current_assets',
    ...
    FROM selected
    WHERE metric IN ('revenue', 'operating_income')
    ...
    SELECT
        'current_asset_turnover'::text AS factor,
        f.asset_id, f.as_of_date, f.signal_month,
        f.revenue_ttm / s.current_assets AS value,
        1::integer AS predicted_sign
    FROM latest_flows f
    JOIN latest_stocks s USING (asset_id, as_of_date)
    WHERE f.revenue_ttm IS NOT NULL AND s.current_assets > 0
    UNION ALL
    SELECT
        'operating_earnings_yield', f.asset_id, f.as_of_date, f.signal_month,
        f.operating_income_ttm / u.market_cap, 1
    FROM latest_flows f
    JOIN universe u
      ON u.asset_id = f.asset_id AND u.trade_date = f.as_of_date
    WHERE f.operating_income_ttm IS NOT NULL AND u.market_cap > 0
    UNION ALL
    SELECT
        'operating_income_to_current_liabilities',
        f.asset_id, f.as_of_date, f.signal_month,
        f.operating_income_ttm / s.current_liabilities, 1
    FROM latest_flows f
    JOIN latest_stocks s USING (asset_id, as_of_date)
    WHERE f.operating_income_ttm IS NOT NULL AND s.current_liabilities > 0
    UNION ALL
    SELECT
        'retained_earnings_to_equity',
        s.asset_id, s.as_of_date, s.signal_month,
        s.retained_earnings / s.total_equity, 1
    FROM latest_stocks s
    WHERE s.retained_earnings IS NOT NULL AND s.total_equity > 0
), ranked AS (
    SELECT
