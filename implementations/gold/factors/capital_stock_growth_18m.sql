-- Query-only legal-capital growth implementation.
-- value = exact 18-month growth in the latest DART PIT capital_stock value.
WITH query_bounds AS (
    SELECT
        (%(start_month)s::date - INTERVAL '18 months')::date AS history_start,
    ...
    SELECT p.asset_id, p.trade_date, p.market, p.market_cap, p.adj_close
    FROM public.factor_price_feature_daily p
    JOIN public.dq_run q
      ON q.run_id = p.quality_run_id
     AND q.status = 'CERTIFIED'
    ...
    SELECT asset_id, as_of_date, signal_month, value AS capital_stock
    FROM stock_candidates
    WHERE metric_rank = 1
), lagged AS (
    SELECT
        latest_stocks.*,
        lag(capital_stock, 18) OVER (asset_months) AS prior_capital_stock,
        lag(signal_month, 18) OVER (asset_months) AS prior_signal_month
    FROM latest_stocks
    WINDOW asset_months AS (PARTITION BY asset_id ORDER BY signal_month)
), raw_values AS (
    SELECT
        lagged.asset_id,
        lagged.as_of_date,
        lagged.signal_month,
        lagged.capital_stock / lagged.prior_capital_stock - 1 AS value
    FROM lagged
    JOIN output_universe u
      ON u.asset_id = lagged.asset_id
     AND u.trade_date = lagged.as_of_date
    CROSS JOIN query_bounds bounds
    WHERE lagged.signal_month BETWEEN bounds.output_start AND bounds.output_end
      AND lagged.capital_stock IS NOT NULL
      AND lagged.prior_capital_stock > 0
      AND lagged.signal_month = lagged.prior_signal_month + INTERVAL '18 months'
), ranked AS (
    SELECT
        asset_id,
        as_of_date,
