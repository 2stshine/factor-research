-- Query-only implementation for realized_daily_volatility_instability_6m.
WITH query_bounds AS (
    ...
    SELECT
        monthly.*,
        stddev_samp(daily_volatility_252d) OVER asset_months
            AS volatility_instability,
        count(daily_volatility_252d) OVER asset_months
            AS volatility_observations
    FROM monthly
    ...
    SELECT
        ...
        signal_month,
        volatility_instability AS value
    FROM universe
    WHERE volatility_observations >= 4
      AND volatility_instability IS NOT NULL
), ranked AS (
    SELECT
        asset_id,
        as_of_date,
