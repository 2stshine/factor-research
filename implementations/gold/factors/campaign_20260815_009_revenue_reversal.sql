-- Query-only revenue-to-noncurrent-assets and three-month reversal implementation for campaign-20260815-009.
-- The certified DART PIT event stream is materialized once for the complete
-- requested interval.
WITH query_bounds AS (
    SELECT
        (%(start_month)s::date - INTERVAL '3 months')::date AS history_start,
    ...
    JOIN public.dq_run q
     ...
     AND q.status = 'CERTIFIED'
    WHERE f.metric IN ('revenue', 'noncurrent_assets')
      AND f.source = 'DART'
    ...
    FROM selected
    WHERE metric = 'noncurrent_assets'
), latest_stocks AS (
    SELECT
        ...
        max(value) FILTER (
            WHERE metric = 'noncurrent_assets' AND metric_rank = 1
    ...
    SELECT
        research_monthly.*,
        lag(adj_close, 3) OVER (asset_months) AS prior_adj_close,
        lag(signal_month, 3) OVER (asset_months) AS prior_signal_month
    FROM research_monthly
    ...
    SELECT
        'short_term_reversal_3m'::text AS factor,
        p.asset_id,
        p.trade_date AS as_of_date,
        p.signal_month,
        p.adj_close / p.prior_adj_close - 1 AS value,
        -1::integer AS predicted_sign
    FROM price_lagged p
    JOIN universe u
      ON u.asset_id = p.asset_id
     AND u.trade_date = p.trade_date
    WHERE p.prior_adj_close > 0
      AND p.signal_month = p.prior_signal_month + INTERVAL '3 months'
), accounting_values AS (
    SELECT
        'revenue_to_noncurrent_assets'::text AS factor,
        f.asset_id, f.as_of_date, f.signal_month,
        f.revenue_ttm / s.noncurrent_assets AS value,
        1::integer AS predicted_sign
    FROM latest_flows f
    JOIN latest_stocks s USING (asset_id, as_of_date)
    WHERE f.revenue_ttm IS NOT NULL
      AND s.noncurrent_assets > 0
    UNION ALL
    SELECT factor, asset_id, as_of_date, signal_month, value, predicted_sign
    FROM reversal_values
), ranked AS (
    SELECT
