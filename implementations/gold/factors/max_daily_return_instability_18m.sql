-- Query-only implementation for max_daily_return_instability_18m.
-- value = 최근 18개 신호월 max_daily_return_1m 의 표본표준편차 (유효 관측 13개 이상, ddof=1)
...
WITH query_bounds AS (
    ...
    SELECT
        monthly.*,
        stddev_samp(max_daily_return_1m) OVER asset_months
            AS max_daily_return_instability_18m,
        count(max_daily_return_1m) OVER asset_months
            AS max_daily_return_instability_observations_18m
    FROM monthly
    ...
    SELECT
        ...
        signal_month,
        max_daily_return_instability_18m AS value
    FROM universe
    WHERE max_daily_return_instability_observations_18m >= 13
      AND max_daily_return_instability_18m IS NOT NULL
), ranked AS (
    SELECT
        asset_id,
        as_of_date,
