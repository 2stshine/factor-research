-- Query-only implementation for book_to_market_change_6m.
WITH query_bounds AS (
    ...
    SELECT
        ...
        max(value) FILTER (WHERE metric_rank = 1) AS total_equity
    ...
    SELECT
        ...
        lag(s.total_equity / m.market_cap, 6) OVER (asset_months)
            AS prior_book_to_market,
        lag(s.signal_month, 6) OVER (asset_months) AS prior_signal_month
    ...
    SELECT
        'book_to_market_change_6m'::text AS factor,
        values.asset_id,
        values.as_of_date,
        values.signal_month,
    ...
    WHERE values.signal_month BETWEEN bounds.output_start AND bounds.output_end
      AND values.book_to_market IS NOT NULL
