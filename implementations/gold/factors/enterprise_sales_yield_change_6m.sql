-- Query-only implementation for enterprise_sales_yield_change_6m.
WITH query_bounds AS (
    ...
    SELECT
        asset_id,
        as_of_date,
        max(value) FILTER (WHERE metric_rank = 1) AS total_liabilities
    ...
    WHERE NOT EXISTS (
        ...
        WHERE explicit_q4.asset_id = fy.asset_id
          ...
          AND explicit_q4.metric = 'revenue'
    ...
    SELECT
        ...
        m.signal_month,
        CASE
            WHEN m.market_cap::double precision + s.total_liabilities > 0
            THEN f.revenue_ttm
                / (m.market_cap::double precision + s.total_liabilities)
        END AS enterprise_sales_yield
    FROM factor_monthly m
    LEFT JOIN flow_ttm f
      ON f.asset_id = m.asset_id
    ...
    SELECT
        accounting_months.*,
        lag(enterprise_sales_yield, 6) OVER (asset_months)
            AS prior_enterprise_sales_yield,
        lag(signal_month, 6) OVER (asset_months) AS prior_signal_month
    ...
    SELECT
        ...
        values.as_of_date,
        values.signal_month,
        values.enterprise_sales_yield
            - values.prior_enterprise_sales_yield AS value
    FROM lagged_values values
    ...
    WHERE values.signal_month BETWEEN bounds.output_start AND bounds.output_end
      AND values.enterprise_sales_yield IS NOT NULL
      AND values.prior_enterprise_sales_yield IS NOT NULL
