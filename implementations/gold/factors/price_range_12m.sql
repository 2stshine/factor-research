-- price_range_12m Gold implementation.
-- value = exact 12 monthly closes' maximum / minimum - 1; sign = -1.
WITH query_bounds AS (
    ...
    SELECT
        research_monthly.*,
        max(adj_close) OVER asset_months AS range_high,
        min(adj_close) OVER asset_months AS range_low,
    ...
    SELECT
        ...
        f.signal_month,
        f.range_high / f.range_low - 1 AS value
    ...
    WHERE f.signal_month BETWEEN bounds.output_start AND bounds.output_end
      ...
      AND f.adj_close > 0
      AND f.range_low > 0
      AND f.signal_month = f.oldest_signal_month + INTERVAL '11 months'
), ranked AS (
    SELECT
        asset_id,
        as_of_date,
